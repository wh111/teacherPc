    ...
    <div class="showHaveCorrected-root">
        ...
        <el-row style="border-bottom: 1px solid #D9E0E2;">
            <div class="rightTitle-Already">
                ...
                <div class="status">
                    <b>{{viewData.totalScore||viewData.score}}分</b>
                </div>
            </div>
            <div class="img">
        ...
        <div class="paperBody">
            <template v-for="(item,questionsTypeId) in viewData.questionsList">
                ...
                <router-link target="_blank"  :to="{name:'groupDetailOfQuestion',query:{types:4,assessmentId:id,questionsId:item.id,studentId:studentId}}">
                <question-show
                        :key="questionsTypeId"
                ...
                >
                    ...
                    <!--<template v-else slot="answerRight">-->
                        <!--<template>-->
                            ...
                            <!--{{item.studentScore}} / {{item.score}}分-->
                        <!--</template>-->
                    <!--</template>-->
                    <template v-if="item.groups">
                        <template  slot="title">
    ...
    methods: {
      ...
      getInfo(){
        this.ajax({
          ...
          ajaxParams: {
            url: api.getAnswerAnalysisInfo.path,
            method: api.getAnswerAnalysisInfo.method,
            params: {
              paperId:this.id,
