...
<template>
    <div class="taskCenter-index">
        <div class="list-wrap">
            ...
            <!--<el-col :span="4">-->
            <!--<router-link v-if="true" :to="{name: 'startTask',params:{id:item.id,type:'setQuestion'}}">-->
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <router-link v-if="scope.row.progressStatus==2"
                                         :to="{name: 'startTask',params:{id:scope.row.id,type:'setQuestion'}}">
                                <el-button class="orangeYellow" type="warning">查看详情</el-button>
                            </router-link>
                            <el-button type="warning" @click="beginTask(scope.row)" v-else style="margin-left: 5px"
                                       class="orangeYellow"    size="small" :disabled="scope.row.progressStatus==0">开始任务
                            </el-button>
    ...
    methods: {
      beginTask (val) {
        if (val.progressStatus == 1) {
          this.$router.push({name: 'startTask', params: {id: val.id, type: 'setQuestion'}})
        }
      },
      //初始化请求列表数据
      init () {
      ...
      brushList () {
        let params = {}
        params = Util._.defaultsDeep({}, this.myPages, {types: 1}, this.formValidate)
        this.ajax({
          ajaxSuccess: res => {
