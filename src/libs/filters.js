  ...
  const filters = [ // 声明全局过滤器及回调函数
    ...
    { // 默认图片
      ...
      call (value) {
        ...
        return '/sasteacher/image/physician.png'
      }
    },
    {
      name: 'formatSize',
    ...
    { // 移除html标签
      ...
      call (html, keepStyleText = false) {
        let resultStr = '';
        if (html) {
          console.log(keepStyleText)
          if (keepStyleText) {
            resultStr = html.replace(/<img[^>]+>/g, '[图片]'); //去掉html
            resultStr = resultStr.replace(/<p><video[^>]*><\/p>/g, '[视频]'); //去掉html
            resultStr = resultStr.replace(/<p><audio[^>]+><\/p>/g, '[音频]'); //去掉html
