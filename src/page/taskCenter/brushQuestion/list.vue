...
<template>
    <div class="taskCenter-index">
        <div class="list-wrap">
            <el-row class="status-type">
                <el-col :span="14" class="btns">
                    状态：
                    <span :class="formValidate.auditStatus === '' ? 'active' : ''"
                          @click="choiceStatus('')">全部</span>
                    <span v-for="(text,index) in ['未审核','审核通过','审核驳回']" :key="index"
                          :class="formValidate.auditStatus === index.toString() ? 'active' : ''"
                          @click="choiceStatus(index)">{{ text }}</span>
                </el-col>
                <el-col :span="10" align="right">
                    <el-button round class="right orangeYellow" type="warning" icon="el-icon-plus" @click="add">创建任务
                    </el-button>
                </el-col>
            </el-row>
            ...
            <div id="myTable" ref="myTable">
                <el-table
                    ...
                    <el-table-column
                            ...
                            align="center">
                        <template slot-scope="scope">
                            <span v-if="scope.row.auditStatus!=1">{{scope.row.auditStatus|auditStatus}}</span>
                            <span v-else>
                                <router-link v-if="scope.row.progressStatus!=0"
                                             :to="{path:'/selfCenter/task_goon',query:{productId:scope.row.id}}">
                                <el-button type="warning" class="orangeYellow"  @click="viewMes(scope.row)">查看详情</el-button>
                                </router-link>
                               <span v-else>
                                   未开始
                               </span>
    ...
    methods: {
      viewMes (val) {
        ...
        if (val.progressStatus == 1) {
          // console.log(val.id,149)
          this.$router.push({path: '/selfCenter/task_goon', query: {productId: val.id}})

          // console.log(val.id,150)
        }
      },
      //初始化请求列表数据
      init () {
      ...
      brushList () {
        let params = {}
        params = Util._.defaultsDeep({}, this.myPages, this.formValidate)
        this.ajax({
          ajaxSuccess: res => {
          ...
          ajaxParams: {
            url: api.gamesAnswerList.path,
            method: api.gamesAnswerList.method,
            params,
          },
        })
      },
      ...
      choiceStatus (status) {
        console.log(status)
        this.formValidate.auditStatus = status.toString()
        this.brushList()
      },
      /***
       * 创建任务
