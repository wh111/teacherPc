...
<template>
    <div class="assignment">
        <h1 class="aTopTitle"><span v-if="type=='homework'">布置作业</span><span v-else>智能组卷</span></h1>
        <div class="aStepBox">
        ...
        <el-row class="aMain">
            ...
            <el-col v-if="step === 0">
                <el-form label-width="100px" ref="formValidate" :model="formValidate" :rules="testPaperManagementZn0">
                    <el-col>
                        ...
                        <el-form-item v-if="type=='homework'" label="作业名称" prop="title">
                            <el-input v-model="formValidate.title" class="amTitleInput"
                                      placeholder="请输入作业名称" @blur="handleHomeWorkTitle"></el-input>
                        </el-form-item>
                        <el-form-item v-else label="试卷名称" prop="title">
                            <el-input v-model="formValidate.title" class="amTitleInput"
                                      placeholder="请输入试卷名称"></el-input>
                        </el-form-item>
                        <el-form-item prop="startTime" label="设置时间" class="el-form--inline amSetTime">
                            <date-group class="amSetTime1"
                                    :dateGroup="{text:'',startDate:formValidate.startTime,endDate:formValidate.submitTime}">
                                <el-form-item slot="start"  label="发布时间" label-width="80px">
                                ...
                                <i></i>
                                <el-form-item slot="end" label="截止时间" label-width="80px"
                                              class="amstEnd">
                            ...
                            <el-form-item class="amSetTime1" prop="publishAnswerTime" label="公布答案" label-width="80px"
                    ...
                    <el-form-item label="试卷结构">
                        <el-table align="center" :context="self" class="questionKnowledgeTable" maxHeight="400"
                            ...
                            <el-table-column label="操作" align="center" width="70">
                                <template slot-scope="scope">
                                    <el-button class="redButton" size="small" type="danger" @click="removeQuestions(scope.$index)">删除
                                    </el-button>
                                </template>
                            </el-table-column>
                        </el-table>
            ...
            <el-col align="center" class="amFooterBtn">
                ...
                <el-button type="success" class="greenButton" @click="next" v-show="step < 2"  >下一步</el-button>
                <load-btn @preview="previewHome" v-show="step === 2" class="greenButton" :btnData="saveBtn" v-if="!['intelligent'].includes(paperType)"></load-btn>
                <load-btn @preview="preview" v-show="step === 2" class="greenButton" :btnData="saveBtn" v-if="!['homework'].includes(paperType)"></load-btn>
                <load-btn @openModal1="openModal1" v-show="step === 2" :btnData="saveTempBtn" style="width: 100px" class="whiteButton"
                          v-if="!['homework'].includes(paperType)"></load-btn>
            </el-col>
        </el-row>
        <!--保存模板弹窗-->
        <Modal :mask-closable="false" height="200" v-model="reserveModal" class-name="reserve-modal"
               :loading="loading" :width="300">
            <modal-header slot="header" :content="reserveModalId"></modal-header>
            <reserve v-if="reserveModal" :title="formValidate.title" @emit="setTempTitle" @cancel="cancel">
            </reserve>
            <div slot="footer"></div>
        </Modal>
    </div>
    ...
    data () {
      return {
        ...
        formValidate: {
          ...
          'publishAnswerTime': new Date(new Date().setDate(new Date().getDate() + 1)), // 公布答案时间，时间戳（智能组卷使用）
          'times': '', // 答题时长
        ...
        homeWorkTitle:true,
        arr:[
          {"id":1,"name":"中心展示","remark":null,"moduleUrl":null,"moduleOrder":null,"isDisplay":1,"isHome":"YES","isFunc":"NO","isPictureNews":"NO","parentId":-105,"parentName":"中心风采","isSingle":"NO","childModuleList":[],"modelUrl":"/portal/page/Teaching-trends.html"},
          {"id":19,"name":"本科生教学","remark":"","moduleUrl":"/portal/page/centralCourse.html","moduleOrder":null,"isDisplay":1,"isHome":"YES","isFunc":"NO","isPictureNews":"NO","parentId":12,"parentName":"教学活动","isSingle":"YES","childModuleList":[],"modelUrl":"/portal/page/Teaching-trends.html"},
          {"id":20,"name":"研究生教学","remark":"","moduleUrl":"/portal/page/centralCourse.html","moduleOrder":null,"isDisplay":1,"isHome":"YES","isFunc":"NO","isPictureNews":"NO","parentId":12,"parentName":"教学活动","isSingle":"YES","childModuleList":[],"modelUrl":"/portal/page/Teaching-trends.html"},
          {"id":21,"name":"住院医师培训","remark":"","moduleUrl":"/portal/page/centralCourse.html","moduleOrder":null,"isDisplay":1,"isHome":"YES","isFunc":"NO","isPictureNews":"NO","parentId":12,"parentName":"教学活动","isSingle":"NO","childModuleList":[],"modelUrl":"/portal/page/Teaching-trends.html"},{"id":22,"name":"特色培训","remark":"","moduleUrl":"/portal/page/centralCourse.html","moduleOrder":null,"isDisplay":1,"isHome":"YES","isFunc":"NO","isPictureNews":"NO","parentId":12,"parentName":"教学活动","isSingle":"NO","childModuleList":[],"modelUrl":"/portal/page/Teaching-trends.html"}]
      }
    },
    methods: {
      handleStartTime (d) {
      ...
      init () {
        setTimeout(()=>{
          this.formValidate['startTime'] = new Date() // 发布时间，时间戳
          this.formValidate['submitTime'] = new Date(new Date().setDate(new Date().getDate() + 1)) // 截止时间，时间戳
        },100)
        Util = this.$util
      ...
      // 下一步
      next () {
        if(!this.formValidate.questionType.length){
          this.errorMess('请选择题型');
          return;
         }
        if(this.formValidate.title && !this.homeWorkTitle){
            this.errorMess('请重新填写作业名称');
            return;
        }
      ...
      removeQuestions (index) {
        let del = this.questions[index]; // questionCopy
        ...
        this.questionCopy = Util._.defaultsDeep([], this.questions);
      },
      // 改变分数类型
      ...
      preview (status) {
        ...
        if (this.$refs['formValidate']&&!this.submitForm('formValidate')) {
          return
        }
        ...
        // 缓存智能组卷数据提供下次“换一批”
        // if(this.formValidate.times!=='' || this.formValidate.times ==0){
...
//        }
      ...
      openModal1 () {
        // console.log('modal')
        // console.log(this.formValidate.times )
        if(this.formValidate.times !==''){
      ...
      * */
      ...
      getTempData () {
        ...
        let {diffLevelCode} = this.selectFormValidate
        let {templateName, grade, score, totalQuestions, scoreType, term} = data
...
//          }
//        }
//        this.chartDate.questionsTypesNameArr = questionsTypesNameArr
//        this.chartDate.questionsTypesDataArr= questionsTypesDataArr
//        this.chartDate.questionsKnowledgesNameArr = questionsKnowledgesNameArr
//        this.chartDate.questionsKnowledgesDataArr = questionsKnowledgesDataArr
//        this.$store.commit('papers/setchartOption', this.chartDate)
...
//        this.questionsKnowledgesDataArr =questionsKnowledgesDataArr// 知识点类型对应的数量信息数组
      ...
      getQuestions () {
        ...
        this.getDataForServer('questions', {
          ...
          if(this.questionCopy.length > 0){
            this.questions.map((item)=>{
              this.questionCopy.map((itemCopy)=>{
                if(item.knowledgeId == itemCopy.knowledgeId){
                  for(var key in itemCopy){
                      item[key] = itemCopy[key];
                  }
                }
              });
            });
      ...
      handleHomeWorkTitle(){
        ...
        if(this.formValidate.title){
          let name = this.formValidate.title;
          let opt = {
            done:(res) => {
                if(res.status && res.status.code != '0'){
                    this.homeWorkTitle = false;
                }else{
                  this.homeWorkTitle = true;
                }
            },
            ajaxParams:{
              url:'/work/findInfoByTitle',
              params:{
                title:name, // 作业名称
              },
            },
    ...
    components: {
      questionDiffLevelOption, reserve
    }
  }
